...
const HomePageContent: React.FC<{
  ...
  error: string | null;
  clearError: () => void;
}> = ({ onFileSelect, error, clearError }) => {
  const steps = [
    {
      icon: "upload" as const,
      title: "Upload Image",
      description:
        "Click or drag & drop your image. We support various formats like PNG, JPG, and more, up to 20MB.",
    },
    {
      icon: "settings" as const,
      title: "Configure Settings",
      description:
        "Choose your desired output format (JPG/PNG) and select a compression level to balance quality and file size.",
    },
    {
      icon: "download" as const,
      title: "Download",
      description:
        "Compress with a single click and download your optimized image, ready for the web.",
    },
  ];
  ...
  const features = [
    {
      ...
      description:
        "Leverages advanced AI to intelligently reduce file size while preserving remarkable image quality.",
    },
    {
      icon: "switch-horizontal" as const,
      title: "Format Conversion",
      description:
        "Easily convert your images between JPG and PNG formats to suit your needs.",
    },
    {
      icon: "eye" as const,
      title: "Live Preview & Edit",
      description:
        "Instantly see the result and compare it with the original. Crop your images for the perfect finish.",
    },
    {
      icon: "shield-check" as const,
      title: "Privacy First",
      description:
        "Your images are processed in memory and never stored on our servers, ensuring your data remains private.",
    },
  ];
  ...
  return (
    <>
      ...
      <section className="py-16 sm:py-24 text-center">
        <div
          className="p-8 bg-gradient-to-r from-brand-purple via-brand-pink to-brand-blue rounded-2xl shadow-2xl animate-background-pan relative overflow-hidden animate-fade-in-up"
          style={{ animationDelay: "300ms" }}
        >
          <div
            className="absolute -top-1/2 -left-1/2 w-[200%] h-[200%] bg-[radial-gradient(circle_at_center,rgba(255,255,255,0.1)_0%,rgba(255,255,255,0)_70%)] animate-spin"
            style={{ animationDuration: "20s" }}
          ></div>
          <div className="relative">
            <Icon
              name="logo"
              className="w-16 h-16 mx-auto text-white/80 mb-4 animate-glow"
            />
            <h2 className="text-3xl font-extrabold text-white">
              Powered by Gemini
            </h2>
            <p className="mt-2 text-lg text-white/90 max-w-2xl mx-auto">
              We use Google's state-of-the-art Gemini model to perform
              intelligent, context-aware image compression that understands
              every pixel.
            </p>
          </div>
        </div>
...
export default function App() {
  ...
  const handleCrop = (
    croppedOriginalB64: string,
    croppedCompressedB64: string
  ) => {
    setOriginalImagePreview(croppedOriginalB64);
    setCompressedImage(croppedCompressedB64);

    const newOriginalFile = dataURLtoFile(
      croppedOriginalB64,
      originalImage?.name ?? `cropped-original.${outputFormat}`
    );
  ...
  return (
    <div
      className={`min-h-screen w-full flex flex-col font-sans text-gray-800 dark:text-slate-200 transition-colors duration-300 relative isolate overflow-hidden`}
    >
