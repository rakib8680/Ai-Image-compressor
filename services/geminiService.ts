...
const ai = new GoogleGenAI({
  apiKey: (window as any).GEMINI_API_KEY as string,
});
  ...
  try {
    ...
    if (textPart?.text) {
      ...
      if (textPart.text.toLowerCase().includes("policy")) {
        throw new Error(
          "The image could not be processed due to content policy reasons."
        );
      }
      throw new Error(
        "The AI model was unable to process this image. It might be an unsupported format or a corrupt file."
      );
    }

    // Fallback error if the response is empty or malformed.
    throw new Error(
      "No image data found in the API response. The model may have failed to generate a compressed image."
    );
  } catch (error) {
    ...
    throw new Error(
      "Failed to communicate with the AI service. Please check your connection and try again."
    );
  }
};
